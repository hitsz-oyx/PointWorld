...
class TrajectoryWindow:
    """An inclusive fixed-size model window over demo action indices."""
...
def plan_trajectory_windows(
    *,
    ...
    last_start = end_idx - window_size + 1
    ...
    return [
        TrajectoryWindow(start_idx=s, end_idx=s + window_size - 1)
        for s in starts
    ]
...
def newly_covered_local_indices(
    ...
    *,
    ...
    local_start = max(0, int(covered_through) + 1 - window.start_idx)
    if local_start > (window.end_idx - window.start_idx):
        return range(0, 0)
    return range(local_start, window.end_idx - window.start_idx + 1)
...
def build_trajectory_frame_map(
    ...
    for window_idx, window in enumerate(sorted(windows, key=lambda item: item.start_idx)):
        for local_idx in newly_covered_local_indices(
            ...
            frame_map[window.start_idx + local_idx] = (window_idx, local_idx)
